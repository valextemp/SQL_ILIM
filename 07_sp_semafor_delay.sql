...
use [Procont]
IF OBJECT_ID ( 'Procont.dbo.sp_semafor_delay', 'P' ) IS NOT NULL   
    DROP PROCEDURE dbo.sp_semafor_delay;  
GO  
CREATE PROCEDURE dbo.sp_semafor_delay  
...
Select @MaxTime = max(DT_Change) FROM [Procont].[dbo].[Semafor_Delay]
...
'		SELECT ef.starttime, ef.endtime, ts.*
		...
		INNER JOIN [AF].[DataT].[ft_TransposeEventFrameSnapshot_Semafor_Delay] ts ON ts.EventFrameID = ef.ID
		WHERE eft.Name = N''Semafor_Delay''
...
FROM [Procont].[dbo].[Semafor_Delay]
where Duration is null

--SELECT * FROM @Current


UPDATE [Procont].[dbo].[Semafor_Delay]
SET  [Duration] = datediff(s,e.starttime,e.endtime)
	,[DT_Change] = e.EndTime
FROM @Event e
where e.ASODU_ID collate SQL_Latin1_General_CP1_CI_AS =[Procont].[dbo].[Semafor_Delay].[ASODU_ID] 
	  and e.Description collate SQL_Latin1_General_CP1_CI_AS = [Procont].[dbo].[Semafor_Delay].[Description] 
	  and e.PV=[Procont].[dbo].[Semafor_Delay].[PV] 
	  and e.StartTime=[DataTime]
	  and e.endtime is not null

INSERT INTO [Procont].[dbo].[Semafor_Delay]
	(
		[ASODU_ID]
		,[Description]
		,[PV]
